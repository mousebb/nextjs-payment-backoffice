...
// Interface for a single withdrawal record
...
const WithdrawalList = ({
  ...
  const internalOnWithdrawalSelect = (withdrawal: ApiWithdrawal) => {
    if (onViewDetail) {
      onViewDetail(withdrawal.id, 'withdrawal', 'withdrawal-list');
    } else {
      handleCopyId(withdrawal.id);
      console.log(
        'Withdrawal selected (default action: copied ID):',
        withdrawal.id
      );
    }
  };
    ...
    const fetchWithdrawals = async () => {
      ...
      try {
        ...
        try {
          ...
          result = { message: 'Failed to fetch withdrawals' };
        }
        if (!response.ok) {
          throw new Error(
  ...
  const withdrawalsToDisplay = useMemo(() => {
    return !enablePagination &&
      displayLimit &&
      withdrawals.length > displayLimit
      ? withdrawals.slice(0, displayLimit)
      : withdrawals;
  }, [withdrawals, enablePagination, displayLimit]);

  const withdrawalListActions: ActionItem[] = [
    {
      label: 'Refresh',
  ...
  if (enablePagination) {
    withdrawalListActions.push({
      label: 'Export',
      icon: <ArrowUpOnSquareIcon />,
      onClick: handleExport,
      disabled: isLoading || withdrawalsToDisplay.length === 0,
    });
  }

  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedIds(new Set(withdrawalsToDisplay.map(item => item.id)));
    } else {
  ...
  const isAllOnPageSelected = useMemo(() => {
    return (
      ...
      withdrawalsToDisplay.every(w => selectedIds.has(w.id))
    );
  }, [withdrawalsToDisplay, selectedIds]);
  ...
  // 表格列定义
  const columns: ListColumn<ApiWithdrawal>[] = [
    {
      key: 'withdrawal_id',
      title: 'Withdrawal ID',
      sortable: showSearchBar,
      render: (v: string, row: ApiWithdrawal) => (
        <div className="flex items-center space-x-1.5">
          <button
            onClick={() => internalOnWithdrawalSelect(row)}
